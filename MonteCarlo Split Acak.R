...
library(ghyp)
library(stats)
library(writexl) # Tambahkan library untuk menyimpan data ke Excel
...
# Membagi data training (80%) dan testing (20%) berdasarkan urutan
n <- nrow(returns_btc)
...
S0_call <- 74315  # Harga aset di tanggal 2024-03-12 (baris terakhir training data)
num_simulations <- nrow(simulated_returns)  # Jumlah simulasi dari baris matrix
num_observations <- ncol(simulated_returns)  # Jumlah langkah waktu dari kolom matrix

# Inisialisasi price_paths matrix
price_paths <- matrix(NA, nrow = num_simulations, ncol = num_observations + 1)
price_paths[, 1] <- S0_call  # Harga awal untuk setiap simulasi

# Menghitung price path berdasarkan simulated_returns
...
plot(1:76, price_paths[1, 1:76], type = "l", col = "blue", ylim = range(price_paths[, 1:76]), 
     xlab = "Hari", ylab = "Harga Bitcoin", main = "Simulasi Jalur Harga Bitcoin (76 Hari Terakhir)")
# Tambahkan jalur harga lain ke plot
for (sim in 2:10000) {  # Misalnya, plotkan 200 jalur simulasi pertama
  lines(1:76, price_paths[sim, 1:76], col = sample(colors(), 1))
}
...
lines(tanggal_simulasi, average_path, col = "green", lwd = 2)
...
# Tentukan rentang untuk sumbu Y yang mencakup kedua data
y_range <- range(c(asset_bitcoin$Last, average_path))  # Menentukan rentang y untuk harga aktual dan simulasi
padding <- diff(y_range) * 0.05  # Menambahkan padding sebesar 5% dari rentang data y

# Menyesuaikan margin dan ukuran plot
par(mar = c(5, 4, 4, 2) + 0.1)  # Menyesuaikan margin plot untuk memberi ruang pada label

# Plot harga aktual aset Bitcoin
plot(tanggal_aset, asset_bitcoin$Last, type = "l", col = "blue", lwd = 2,
     xlab = "Tanggal", ylab = "Harga Bitcoin",
     main = "Harga Aset Bitcoin dan Rata-rata Simulasi GH vs Normal",
     xlim = range(tanggal_aset), ylim = c(min(y_range) - padding, max(y_range) + padding))
...
library(writexl)
file_path <- "/Users/shannenwibisono/Desktop/-SKRIPSI-/Results MC/80:20/Hasil MC8020 BTC.xlsx"
...
output_call_prices_path <- "/Users/shannenwibisono/Desktop/-SKRIPSI-/Monte Carlo/Kumpulan_Price_BTC_8020.xlsx"
...
input_call_prices_path <- "/Users/shannenwibisono/Desktop/-SKRIPSI-/Monte Carlo/Kumpulan_Price_BTC_8020.xlsx"
...
output_average_prices_path <- "/Users/shannenwibisono/Desktop/-SKRIPSI-/Monte Carlo/Average_Prices_BTC_8020.xlsx"
...
output_normality_results_path <- "/Users/shannenwibisono/Desktop/-SKRIPSI-/Monte Carlo/Normality_Results_BTC_8020.xlsx"
...
output_ci_path <- "/Users/shannenwibisono/Desktop/-SKRIPSI-/Monte Carlo/Confidence_Intervals_Per_Day_BTC_8020.xlsx"
...
output_put_prices_path <- "/Users/shannenwibisono/Desktop/-SKRIPSI-/Monte Carlo/Kumpulan_PricePUT_BTC_8020.xlsx"
...
input_put_prices_path <- "/Users/shannenwibisono/Desktop/-SKRIPSI-/Monte Carlo/Kumpulan_PricePUT_BTC_8020.xlsx"
...
output_average_prices_path <- "/Users/shannenwibisono/Desktop/-SKRIPSI-/Monte Carlo/Average_PricesPUT_BTC_8020.xlsx"
write_xlsx(list("Average Prices" = average_prices_per_group), path = output_average_prices_path)
cat("Rata-rata harga per grup telah disimpan ke:", output_average_prices_path, "\n")
...
# Simpan hasil uji normalitas ke Excel
output_normality_results_path <- "/Users/shannenwibisono/Desktop/-SKRIPSI-/Monte Carlo/Normality_Results_PUT_BTC_8020.xlsx"
write_xlsx(list("Normality Results" = normality_results), path = output_normality_results_path)
...
# Simpan hasil confidence interval ke file Excel
output_ci_path <- "/Users/shannenwibisono/Desktop/-SKRIPSI-/Monte Carlo/Confidence_Intervals_PUT_Per_Day_BTC_8020.xlsx"
write_xlsx(list("Confidence Intervals" = confidence_intervals_per_day), path = output_ci_path)
cat("Confidence intervals per day telah disimpan ke:", output_ci_path, "\n")
...
price_data <- read_excel("/Users/shannenwibisono/Desktop/-SKRIPSI-/Monte Carlo/BTC8020/Kumpulan_Price_BTC_8020.xlsx")
...
write.xlsx(quantile_df, "/Users/shannenwibisono/Desktop/-SKRIPSI-/Monte Carlo/BTC8020/Percentile_BTCCall_8020.xlsx")
...
priceput_data <- read_excel("/Users/shannenwibisono/Desktop/-SKRIPSI-/Monte Carlo/BTC8020/Kumpulan_PricePUT_BTC_8020.xlsx")
...
# 6. Simpan hasil ke file Excel baru
write.xlsx(quantileput_df, "/Users/shannenwibisono/Desktop/-SKRIPSI-/Monte Carlo/BTC8020/Percentile_BTCPut_8020.xlsx")
...
pricepath_data <- read_excel('/Users/shannenwibisono/Desktop/-SKRIPSI-/Monte Carlo/BTC8020/Pricepath_BTC_8020.xlsx')

# 2. Baca data actual
asset_bitcoin <- read_excel('/Users/shannenwibisono/Desktop/-SKRIPSI-/Bitcoin/BTM24 OPSI FIX.xlsx', sheet = "Asset Price R")

# 3. Bagi data menjadi 80% training dan 20% testing
...
ggplot(data = NULL, aes(x = tanggal_aset)) +
  ...
  labs(
    title = "Harga Aset BTC (Testing 20%) dan Percentile 5%-95%",
    x = "Tanggal",
    y = "Harga",
    color = "Legenda"
  ) +
  scale_y_continuous(limits = c(min(c(actual_data$Last, percentile_df$Percentile_5)) * 0.9, 
                                max(c(actual_data$Last, percentile_df$Percentile_95)) * 1.1)) +
  theme_minimal()
...
ggplot() +
  ...
  # Label dan tampilan
  labs(
    title = "Harga Aset BTC Rasio 80:20 (Training vs Testing)",
    x = "Tanggal",
    y = "Harga",
    color = "Legenda"
  ) +
...
write.xlsx(percentile_df, "/Users/shannenwibisono/Desktop/-SKRIPSI-/Monte Carlo/BTC8020/Percentile_Result_AsetBTC_8020.xlsx")
